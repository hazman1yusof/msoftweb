...
class inventoryRequestExport implements FromView, WithEvents, WithColumnWidths
{
    ...
    */
    ...
    public function __construct($reqdept_from,$reqdept_to,$datefr,$dateto)
    {
        $this->reqdept_from = $reqdept_from;
        $this->reqdept_to = $reqdept_to;
        $this->datefr = $datefr;
        $this->dateto = $dateto;
    ...
    {  
        $reqdept_from = $this->reqdept_from;
        if(empty($this->reqdept_from)){
            $reqdept_from = '%';
        }
        $reqdept_to = $this->reqdept_to;
        ...
        $ivrequest = DB::table('material.ivreqhd as h')
                ->select('h.idno', 'h.compcode', 'h.recno as h_recno', 'h.reqdt', 'h.ivreqno', 'h.reqdept', 'h.reqtodept', 'h.recstatus', 'd.recno as d_recno','d.itemcode', 'd.uomcode', 'd.pouom', 's.qtyonhand', 'd.qtyrequest', 'd.qtybalance', 'd.qtytxn', 'd.netprice', 'd.ivreqno', 'd.reqdept', 'p.description')                ->join('material.ivreqdt as d', function($join){
                ...
                ->get();
        // dd($ivrequest);
        return view('material.inventoryRequest.inventoryRequest_excel',compact('ivrequest'));
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                .sprintf('FROM %s TO %s',$this->reqdept_from, $this->reqdept_to)
                .'&L'
