...
use Mail;
use Carbon\Carbon;
use App\Mail\sendmailDP;
use DB;

class SendEmailDP implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($data as $key => $value) {
            $email = $value->email;
            $subject = 'Direct Payment to be '.$value->trantype;
            $type = 'Direct Payment';
            $recno = $value->recno;
            ...
            if(!empty($email) && $NOT_USE_MAIL == null){
                Mail::to($email)->send(new sendmailDP($subject,$type,$recno,$curr_stats,$reqdept,$prepredon));
            }
        }
    }
}
