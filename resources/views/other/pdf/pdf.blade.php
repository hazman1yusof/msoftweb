...
<html dir="ltr" mozdisallowselectionprint>
  ...
  <body tabindex="0">
    ...
    <input type="hidden" id="episno" value="{{$episno}}">
    <input type="hidden" id="type" value="{{$type}}">
    <input type="hidden" id="entereddate" value="{{$entereddate}}">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    button.addEventListener("click", async function () {
      ...
      try {
          ...
          const editedPdf = await pdfjsLib.getDocument({
                  data: pdfData
              }).promise;

          const page = await editedPdf.getPage(1);
...
window.addEventListener('beforeunload', () => {
  if (window.opener && !window.opener.closed) {
    window.opener.reloadImage_png([type],mrn,episno,entereddate); // Call a function defined in the parent
  }
});
