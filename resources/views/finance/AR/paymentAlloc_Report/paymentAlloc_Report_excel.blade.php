<table>
    ...
    @foreach ($dballoc as $obj)
        <tr>
            ...
            <td>{{$obj->reference}}</td>
            <td data-format="0.00" style="text-align: right">{{number_format($obj->dh_amount, 2, '.', ',')}}</td>
            <td>{{$obj->refauditno}}</td>
            <td>{{\Carbon\Carbon::parse($obj->ref_entrydate)->format('d/m/Y')}}</td>
            <td data-format="0.00" style="text-align: right">{{number_format($obj->allocamount, 2, '.', ',')}}</td>
