<script>
		...
		$(document).ready(function () {
			...
			$("#jqGrid").jqGrid({
				...
				datatype: "json",
				 colModel: [
					...
					{label: 'Issue Type', name: 'isstype', width: 90 ,  classes: 'wrap' , editable: true ,
					editrules:{ required: true}, 
					},
					
					{label: 'Transaction By Inventory', name: 'trbyiv', width: 90 ,  classes: 'wrap' , hidden:true,  editable: true , 
					},
					
					{label: 'Update Quantity', name: 'updqty', width: 90 ,  classes: 'wrap' , hidden: true, editable: true, 
					},
					
					{label: 'Credit/Debit', name: 'crdbfl', width: 90 ,  hidden: true, editable: true,
					},
					
					
					
					{label: 'Update GL', name: 'updamt', width: 90 , hidden: true, editable: true ,
					},
					
				
					
					{label: 'Account Type', name: 'accttype', width: 90 , editable: true ,
					editrules:{ required: true}, 
					},
			...
			function saveFormdata(oper){
				$.post( "tranTypeSave.php", $( "#formdata" ).serialize()+'&'+$.param({ 'oper': oper }) , function( data ) {
			...
			function search(Stext,Scol){
				$("#jqGrid").jqGrid('setGridParam',{datatype:'json',url:'tranTypeTbl.php?Scol='+Scol+'&Stext='+Stext}).trigger('reloadGrid');
			}
