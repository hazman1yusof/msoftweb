...
$(document).ready(function () {
	...
	$("#jqGrid").jqGrid({
		...
		colModel: [
			...
			//{ label: 'Request No', name: 'purordhd_purreqno', width: 10, hidden: true },
	...
	function searchChange() {
		...
		}, { fct: ['purordhd.compcode', 'purordhd.recstatus', 'purordhds.prdept'], fv: [], fc: [] });
	...
	var urlParam2 = {
		action: 'get_table_default',
		field: ['podt.compcode', 'podt.recno', 'podt.lineno_', 'podt.pricecode', 'podt.itemcode', 'p.description','podt.uomcode', 'podt.qtyorder', 'podt.qtydelivered', 'podt.perslstax', 'podt.unitprice', 'podt.taxcode', 'podt.perdisc', 'podt.amtdisc','podt.amtslstax', 'podt.amount','NULL AS remarks_button','podt.remarks'],
	...
	$("#jqGrid2").jqGrid({
		...
		colModel: [
            ...
            { label: 'Total Line Amount', name: 'amount', width: 80, classes: 'wrap', editable: true , align: 'right'}, 
...
*/
	...
	var dialog_reqdept = new ordialog(
		'reqdept', 'material.purreqhd p ' ,'#purordhd_reqdept', errorField,

		{
			colModel: [
			    { label: 'Request Department', name: 'reqdept', width: 100, classes: 'pointer', canSearch: true, or_search: true  },
				//{ label: 'Description', name: 'description', width: 400, classes: 'pointer', canSearch: true, or_search: true },
			]
		}, {
			title: "Select Request Department",
			open: function () {
				dialog_reqdept.urlParam.filterCol = ['recstatus','purordno'];
				dialog_reqdept.urlParam.filterVal = ['POSTED', 'IS NULL'];
			}
		}
	);
	...
	var dialog_purreqno = new ordialog(
		'purreqno',['material.purreqhd h','material.purreqdt d'],'#purordhd_purreqno',errorField,
		{	colModel:[
				{label:'Request No',name:'h.purreqno',width:200,classes:'pointer',canSearch:true,checked:true,or_search:true},
				...
				{label:'recno',name:'d.pricecode',width:400,classes:'pointer',hidden:true},
			...
			ondblClickRow: function () {
              ...
              // let newval = $("#purorddt_pricecode").val();
				//$("#jqGrid2").jqGrid('setRowData', dialog_purreqno ,{pricecode:data['d.pricecode']});
				$("#jqGrid2").jqGrid('setRowData', dialog_purreqno ,{pricecode:data['d.pricecode']});
				$("#jqGrid2").jqGrid('addRowData', dialog_purreqno ,{pricecode:data['d.pricecode']});
			...
			open: function(){
				...
				dialog_purreqno.urlParam.filterCol = ['h.reqdept'];
				dialog_purreqno.urlParam.filterVal = [$("#purordhd_reqdept").val()];
				...
				//dialog_purreqno.urlParam.join_filterCol = [['h.recno'],[]];
				//dialog_purreqno.urlParam.join_filterVal = [['skip.d.recno'],[]];
	...
	var dialog_itemcode = new ordialog(
		'itemcode', ['material.stockloc s', 'material.product p','hisdb.taxmast t'], "#jqGrid2 input[name='itemcode']", errorField,
		{
			colModel:
			[
			...
			open: function () {
				...
				dialog_itemcode.urlParam.filterVal = ['session.company', moment($('#purordhd_purdate').val()).year(), $('#purordhd_prdept').val()];
				dialog_itemcode.urlParam.join_type = ['LEFT JOIN','LEFT JOIN'];
				dialog_itemcode.urlParam.join_onCol = ['s.itemcode','p.taxcode'];
				dialog_itemcode.urlParam.join_onVal = ['p.itemcode','t.taxcode'];
