...
$(document).ready(function () {
	...
	$("#jqGrid").jqGrid({
		...
		colModel: [
			...
			// { label: 'Debtor Code', name: 'db_payercode', width: 35, canSearch: false, formatter: showdetail, unformat: un_showdetail },
			{ label: 'Debtor Code', name: 'db_payercode', width: 14, canSearch: true },
			{ label: 'Debtor Name', name: 'dm_name', width: 25, canSearch: true, classes: 'wrap',checked:true },
			...
			{ label: 'source', name: 'db_source', width: 10, hidden: true },
			{ label: 'PO Date', name: 'db_podate', width: 10, formatter: dateFormatter, unformat: dateUNFormatter },
	...
	$("#jqGrid").jqGrid('navGrid', '#jqGridPager', {
		...
		onClickButton: function () {
			...
			if(selrowData("#jqGrid").db_trantype != 'IN'){
				return false;
			}
	...
	function searchChange(once=false){
		...
		if(once){
			...
			if($('#searchForm [name=Stext]').val().trim() != ''){
				...
				urlParam.wholeword=$('#searchForm [name=Stext]').val().trim();
				urlParam.filterCol = filter.fc;
				urlParam.filterVal = filter.fv;
			}
			once=false;
	...
	$("#jqGrid2").jqGrid({
		...
		colModel: [
			...
			{
				label: 'Item Code', name: 'chggroup', width: 150, classes: 'wrap', editable: true,
			...
			{ label: 'Item Description', name: 'description', width: 200, classes: 'wrap'},
			...
			{ label: 'uom_ori', name: 'uom_ori', hidden:true },
			{
				...
				label: 'UOM Code<br/>Store Dept.', name: 'uom_recv', width: 150, classes: 'wrap', hidden: true,
				// editrules: { required: true, custom: true, custom_func: cust_rules },
				// formatter: showdetail,
				// edittype: 'custom', editoptions:
				// {
 ...
 //  	useColSpanStyle: false, 
	...
	function formatterCheckbox(cellvalue, options, rowObject){
		...
		let scompcode = $('#session_compcode').val();
		...
		if($('#recstatus_use').val() == 'ALL'){
			if(rowObject.db_recstatus == "OPEN" && rowObject.db_compcode == scompcode){
				return "<input type='checkbox' name='checkbox_selection' id='checkbox_selection_"+rowObject[idno]+"' data-idno='"+rowObject[idno]+"' data-rowid='"+options.rowId+"'>";
			}else if(rowObject.db_recstatus == "RECOMPUTED"){
				return "<input type='checkbox' name='checkbox_selection' id='checkbox_selection_"+rowObject[idno]+"' data-idno='"+rowObject[idno]+"' data-rowid='"+options.rowId+"'>";
			}
		}else if($('#recstatus_use').val() == 'DELIVERED'){
			if(rowObject.db_recstatus == "PREPARED" && rowObject.db_compcode == scompcode){
				return "<input type='checkbox' name='checkbox_selection' id='checkbox_selection_"+rowObject[idno]+"' data-idno='"+rowObject[idno]+"' data-rowid='"+options.rowId+"'>";
			}
		}else if($('#recstatus_use').val() == 'REOPEN'){
			if(rowObject.db_recstatus == "CANCELLED" && rowObject.db_compcode == scompcode){
				return "<input type='checkbox' name='checkbox_selection' id='checkbox_selection_"+rowObject[idno]+"' data-idno='"+rowObject[idno]+"' data-rowid='"+options.rowId+"'>";
			}
		}else if($('#recstatus_use').val() == 'RECOMPUTED'){
			if(rowObject.db_recstatus == "POSTED" && rowObject.db_compcode == scompcode){
				return "<input type='checkbox' name='checkbox_selection' id='checkbox_selection_"+rowObject[idno]+"' data-idno='"+rowObject[idno]+"' data-rowid='"+options.rowId+"'>";
			}
	...
	var dialog_CustomerSO = new ordialog(
		'customer', 'debtor.debtormast', '#db_debtorcode', errorField,
		{
			...
			ondblClickRow: function () {
				...
				$('#db_hdrtype').val(data['billtypeop']);
				dialog_billtypeSO.check(errorField);
			},
			gridComplete: function(obj){
				var gridname = '#'+obj.gridname;
	...
	var dialog_doctor = new ordialog(
		'dialog_doctor', 'hisdb.doctor', '#db_doctorcode', 'errorField',
		{
			colModel: [
	...
	var dialog_chggroup = new ordialog(
		...
		{	colModel:
			...
			ondblClickRow:function(event){
			    ...
			    $("#jqGrid2").jqGrid('setRowData', id_optid ,{description:data['description']});
			...
			close: function(obj_){
				$("#jqGrid2 #"+obj_.id_optid+"_chggroup").parent().siblings( ".help-block" ).html("");
				$("#jqGrid2 input[name='quantity']").focus().select();
			},
...
function receipt_class(){
	...
	$('#submit_receipt').click(function(){
		...
		$.post( './SalesOrder/form', serializedForm+'&'+$.param(obj)  , function( data ) {
			// refreshGrid('#jqGrid', urlParam);
		},'json').fail(function(data) {
			...
			grid='#jqGrid';
			$('#AlloDtype').val('RC');
			$('#AlloDtype2').html('RECEIPT');
			$('#AlloDno').val(data.recptno);
			$('#AlloDebtor').val(data.payercode);
			$('#AlloDebtor2').html(data.payername);
			$('#AlloPayer').val(data.payercode);
			$('#AlloPayer2').html(data.payername);
			$('#AlloAmt').val(data.amount);
			$('#AlloOutamt').val(data.amount);
			$('#AlloBalance').val(data.amount);
			$('#AlloTotal').val(0);
			$('#AlloAuditno').val(data.auditno);
			urlParamAllo.filterVal[0]=data.payercode;
			refreshGrid("#gridAllo",urlParamAllo);
			myallocation.renewAllo(data.amount);
