...
$(document).ready(function () {
	...
	var urlParam2={
		...
		field:['apdt.compcode','apdt.source','apdt.reference','apdt.trantype','apdt.auditno','apdt.lineno_','apdt.deptcode','apdt.category','apdt.document', 'apdt.AmtB4GST', 'apdt.GSTCode', 'apdt.taxamt as tot_gst','apdt.amount', 'apdt.dorecno', 'apdt.grnno', 'apdt.idno'],
	...
	$("#jqGridAlloc").jqGrid({
		...
		colModel: [
			...
			{ label: 'idno', name: 'idno', width: 80, classes: 'wrap', hidden:true, key: true},
	...
	var dialog_suppcode = new ordialog(
		...
		{	colModel:[
			...
			ondblClickRow:function(){
				...
				// 	}, 'json').done(function (data) {
				// 		if (!$.isEmptyObject(data.rows)) {
				// 			myerrorIt_only2(dialog_suppcode.textfield,false);
...
function calculate_total_alloc(rowid){
	...
	var tot_detail = $('#apacthdr_amount').val();
	...
	if(!isNaN(tot_alloc)){
		...
		if(!isNaN(outamount)){
			...
			if(outamount < 0){
				alert('Allocate amount cant exceed total amount');
				if(rowid != undefined){
					text_error1('input#'+rowid+'_allocamount');
				}
			}else{
				if(rowid != undefined){
					text_success1('input#'+rowid+'_allocamount');
				}
			}
		}
	}
