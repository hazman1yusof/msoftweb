...
$(document).ready(function () {
    ...
    var DataTable = $('#job_queue').DataTable({
        ajax: './ARAgeingColl_Report/table?action=job_queue',
        pageLength: 10,
        orderMulti: false,
        responsive: true,
        scrollY: 500,
        ...
        paging: true,
        columns: [
            { data: 'idno' ,visible:true,orderable: false},
            ...
            {targets: 10,
                createdCell: function (td, cellData, rowData, row, col) {
                    if(rowData.status == 'DONE'){
                        $(td).append(`<a class='btn btn-sm btn-default' target="_blank" href='./ARAgeingColl_Report/table?action=download&idno=`+rowData.idno+`'><i class='fa fa-download'></i></span>`);
                    }
                }
            },
        ],
