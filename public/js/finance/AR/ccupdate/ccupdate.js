...
$(document).ready(function (){
	...
	$("#jqGrid").jqGrid({
		...
		colModel: [
			...
			{ label: 'Debtor Name', name: 'dm_name', width: 28, classes: 'wrap text-uppercase', canSearch: true },
			...
			{ label: 'Invoice No', name: 'db_invno', hidden: true },
			...
			{ label: 'Document No', name: 'db_recptno', width: 18, align: 'right', canSearch: true, selected: true },
			...
			{ label: 'Type', name: 'db_trantype', width: 5, },
			...
			{ label: 'MRN', name: 'db_mrn', width: 10, align: 'right', classes: 'wrap text-uppercase', formatter: showdetail, unformat: un_showdetail },
	...
	$("#dialogForm_updcc")
		.dialog({
			...
			autoOpen: false,
			open: function (event, ui){
				dialog_paymode_updcc.on();
				let rowdata = selrowData('#jqGrid');

				$('#dialogForm_updcc input[name=db_idno]').val(rowdata.db_idno);
				$('#dialogForm_updcc input[name=db_recptno]').val(rowdata.db_recptno);
				$('#dialogForm_updcc input[name=db_posteddate]').val(rowdata.db_posteddate);
				$('#dialogForm_updcc input[name=db_payercode]').val(rowdata.db_payercode);
				$('#dialogForm_updcc span.help-block.db_payercode').text(rowdata.dm_name);
				$('#dialogForm_updcc input[name=db_amount]').val(rowdata.db_amount);
				$('#dialogForm_updcc input[name=db_paymode]').val(rowdata.db_paymode);
			},
			close: function (event, ui){
				dialog_paymode_updcc.off();
				$('#dialogForm_updcc input[name=db_idno]').val('');
				$('#dialogForm_updcc input[name=db_recptno]').val('');
				$('#dialogForm_updcc input[name=db_posteddate]').val('');
				$('#dialogForm_updcc input[name=db_payercode]').val('');
				$('#dialogForm_updcc span.help-block.db_payercode').text('');
				$('#dialogForm_updcc input[name=db_amount]').val('');
				$('#dialogForm_updcc input[name=db_paymode]').val('');
			},
			buttons: [{
				text: "Save", click: function (){
					if(!$('#dialogForm_updcc input[name=db_paymode]').hasClass('valid')){
						alert('Insert correct paymode');
					}else{
						var obj = {
							_token: $('#csrf_token').val(),
							idno: $('#dialogForm_updcc input[name=db_idno]').val(),
							paymode: $('#dialogForm_updcc input[name=db_paymode]').val()
						}
	...
	var dialog_paymode_updcc = new ordialog(
		'db_paymode_updcc', 'debtor.paymode', "#dialogForm_updcc input[name='db_paymode']", errorField,
		{
			colModel: [
	...
	dialog_paymode_updcc.makedialog(false);
	...
	$("#jqGrid_Tracking").inlineNav('#jqGridPager_Tracking', {
				...
				bootbox.confirm({
					...
					callback: function (result){
						if(result == true){
							...
							$.post("./ccupdate/form?"+$.param(param), {oper: 'del', "_token": $("#_token").val()}, function (data){
							}).fail(function (data){
		...
		onClickButton: function (){
			...
			$.post("/ccupdate/form?"+$.param(param), {oper: 'edit_all', dataobj: jqGrid_Tracking_data}, function (data){
			}).fail(function (data){
	...
	function showdetail(cellvalue, options, rowObject){
		...
		fdl.get_array('ccupdate',options,param,case_,cellvalue);
	...
	var customer_search = new ordialog(
		'customer_search', 'debtor.debtormast', '#customer_search', 'errorField',
		{
			...
			ondblClickRow: function (){
				...
				$.get("./ccupdate/table?" + $.param(param), function (data){
	...
	$("#ARStatementDialog").dialog({
		...
		[{
			text: "Generate PDF", click: function (){
				window.open('./ccupdate/showpdf?debtorcode_from='+$('#debtorcode_from').val()+'&debtorcode_to='+$("#debtorcode_to").val()+'&datefr='+$("#datefr").val()+'&dateto='+$("#dateto").val(), '_blank');
			...
			text: "Generate Excel", click: function (){
				...
				window.location='./ccupdate/showExcel?debtorcode_from='+$('#debtorcode_from').val()+'&debtorcode_to='+$("#debtorcode_to").val()+'&datefr='+$("#datefr").val()+'&dateto='+$("#dateto").val();
			}
		},{
			text: "Cancel", click: function (){
				$(this).dialog('close');
