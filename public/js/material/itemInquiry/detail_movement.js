		...
		$(document).ready(function () {
			...
			var dialog_date = new ordialog(
				'date','material.ivtxndt','#date',errorField,
				{	colModel:[
			...
			var urlParam={
				...
				filterVal:['session.company', $('#itemcode').val(), $('#deptcode').val()]
			}
			...
			function populateTable(){
				...
				$.each(rowData, function( index, value ) {
					if(value){
						$('#TableDetailMove #'+index+' span').text();
					}else{
						$('#TableDetailMove #'+index+' span').text();
					}
				});
			...
			$('#search').click(function(){
				urlParam.filterCol = ['ivtxndt.itemcode','ivtxndt.deptcode'];
			...
			var DataTable = $('#TableDetailMove').DataTable({
			    responsive: true,
				scrollY: 500,
				paging: false,
			    columns: [
