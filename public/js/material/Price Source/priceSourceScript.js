		...
		$(document).ready(function () {
			...
			var err_reroll = new err_reroll('#jqGrid',['pricecode', 'description']);
			...
			$("#jqGrid").jqGrid({
				...
				editurl: "./priceSource/form",
				 colModel: [
					...
					{ label: 'Price Code', name: 'pricecode', width: 90, classes: 'wrap', canSearch: true,editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase"}}, 
					{ label: 'Description', name: 'description', width: 90, classes: 'wrap', checked:true, canSearch: true, editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase"}}, 
			...
			var myEditOptions_edit = {
				...
				oneditfunc: function (rowid) {
					...
					$("#jqGridPagerDelete,#jqGridPagerRefresh").hide();
					$("input[name='pricecode']").attr('disabled','disabled');
					$("input[name='description']").keydown(function(e) {//when click tab at last column in header, auto save
						var code = e.keyCode || e.which;
						if (code == '9')$('#jqGrid_ilsave').click();
						/*addmore_jqgrid.state = true;
