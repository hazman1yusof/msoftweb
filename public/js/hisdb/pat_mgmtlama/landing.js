...
$(document).ready(function() {
    ...
    var grid = $("#grid-command-buttons").bootgrid({
        ...
        formatters: {
            "col_add": function (column,row) {
                // return "X"
                return "<button title='Address' type='button' class='btn btn-xs btn-default btn-md command-add' data-row-id=\"" + row.mrn + "\"  name=\"cmd_add" + row.mrn + "\"><span class=\"glyphicon glyphicon-home\" aria-hidden=\"true\"></span></button>";
                // return ('0000000' + row.mrn).slice(-7);
            },
            "col_mrn": function (column,row) {
                return ('0000000' + row.mrn).slice(-7);
            },
            "col_name": function (column,row) {
                return (row.name);
            ...
            "commands": function (column,row) {
                return "<button title='Edit Patient' type='button' class='btn btn-xs btn-warning btn-md command-edit' data-row-id=\"" + row.mrn + "\"  id=\"cmd_edit" + row.mrn + "\"><span class='glyphicon glyphicon-edit' aria-hidden='true'></span></button> " +
                       "<button title='New Episode' type='button' class='btn btn-xs btn-danger btn-md command-episode' data-row-id=\"" + row.mrn + "\"  id=\"cmd_history" + row.mrn + "\"><span class='glyphicon glyphicon-header' aria-hidden='true'></span></button>" +
					   "<button title='New OTC Episode' type='button' class='btn btn-xs btn-danger btn-md command-otc-episode' data-row-id=\"" + row.mrn + "\"  id=\"cmd_otc" + row.mrn + "\"><span class='fa fa-user-md' aria-hidden='true'></span></button>";
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        grid.find(".command-edit").on("click", function(e){
            ...
            // detailRows =
            detailRows = address_details($(this).data("rowId"));
    ...
    function populate_patient_episode (d) {

        if (d > 0) {
            $.ajax({
                type: 'GET',
                url: '../../../../assets/php/entry.php?action=get_patient_detail',
                data: { patid: d },
                dataType: 'json',
                error: function() {
                ...
                success: function(data) {
                    // console.log(data.patient[0].mrn);

                    $('#hid_pat_title').val(data.patient[0].titlecode);
                    $('#mrn').val(('0000000' + data.patient[0].mrn).slice(-7));
                    // $('#oldmrn').val(data.patient[0].oldmrn);

                    var first_visit = new Date(data.patient[0].first_visit_date);
                    ...
                    var last_visit = new Date(data.patient[0].last_visit_date);
                    ...
                    $('#episno').val(data.patient[0].episno); // dlm modal Patient   
					
                    $('#txt_pat_name').val(data.patient[0].name);
                    $('#txt_pat_newic').val(data.patient[0].newic);
                    $('#txt_pat_oldic').val(data.patient[0].oldic);
                    $('#cmb_pat_idtype').val(data.patient[0].id_type);
                    $('#txt_pat_idno').val(data.patient[0].id_number);

                    // $('#occupcode').val(data.patient[0].occupcode);
                    $('#hid_pat_occupation').val(data.patient[0].occupcode);

                    var dob = new Date(data.patient[0].dob);
                    var dob_val = (isNaN(dob.getFullYear()) ? 'n/a' : moment(dob).format('DD/MM/YYYY'));
                    $('#txt_pat_dob').val(dob_val);
                    var day = new Date();
                    var dob = new Date(dob);
                    var age_val =  day.getFullYear() - dob.getFullYear();
                    $('#txt_pat_age').val(age_val);
                    $('#txt_pat_telh').val('0' +data.patient[0].tel_h);
                    $('#txt_pat_telhp').val('0' +data.patient[0].tel_hp);
                    $('#txt_pat_email').val(data.patient[0].Email);
                    $('#hid_pat_area').val(data.patient[0].areacode);
                    $('#cmb_pat_sex').val(data.patient[0].sex); // dlm modal Patient                    
                    $('#hid_pat_citizen').val(data.patient[0].citizencode);
                    $('#cmb_pat_racecode').val(data.patient[0].racecode);
                    $('#cmb_pat_religion').val(data.patient[0].religion);
                    $('#maritalcode').val(data.patient[0].maritalcode);
                    $('#cmb_pat_langcode').val(data.patient[0].languagecode);
                    $('#remarks').val(data.patient[0].remarks);
                    $('#relatecode').val(data.patient[0].relatecode);
                    $('#corpcomp').val(data.patient[0].corpcomp);
                    $('#email_official').val(data.patient[0].email_official);
                    $('#childno').val(data.patient[0].childno);
                    $('#txt_pat_curradd1').val(data.patient[0].address1);
                    $('#txt_pat_curradd2').val(data.patient[0].address2);
                    $('#txt_pat_curradd3').val(data.patient[0].address3);
                    $('#txt_pat_offadd1').val(data.patient[0].offadd1);
                    $('#txt_pat_offadd2').val(data.patient[0].offadd2);
                    $('#txt_pat_offadd3').val(data.patient[0].offadd3);
                    $('#txt_pat_padd1').val(data.patient[0].padd1);
                    $('#txt_pat_padd2').val(data.patient[0].padd2);
                    $('#txt_pat_padd3').val(data.patient[0].padd3);
                    $('#txt_pat_currpostcode').val(data.patient[0].postcode);
                    $('#txt_pat_offpostcode').val(data.patient[0].offpostcode);
                    $('#txt_pat_ppostcode').val(data.patient[0].ppostcode);
                    $('#active').val(data.patient[0].active);
                    $('#confidential').val(data.patient[0].confidential);
                    $('#mrfolder').val(data.patient[0].mrfolder);
                    $('#patientcat').val(data.patient[0].patientcat);
                    $('#newmrn').val(data.patient[0].newmrn);
                    $('#blood_grp').val(data.patient[0].blood_grp);
                    // $('#name').val(data.patient[0].name);

                    // ********* episode ************

                    $('#txt_epis_no').val(parseInt(data.patient[0].episno) + 1); // dlm modal Episode
                    $('#txt_epis_type').val("OP");                    
                    $('#txt_epis_date').val(moment().format('DD/MM/YYYY'));
                    $('#txt_epis_time').val(moment().format('hh:mm:ss'));

                    if (data.patient[0].sex == "M") // dlm modal Episode
                    {
			...
			populate_payer_guarantee_info(d);
        }
    }
        ...
        if (d > 0) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    html = html + 'Phone (Mobile): 0' + data.patient[0].txt_pat_currtelh + '<BR/>';
                    html = html + 'Phone (House): 0' + data.patient[0].txt_pat_currtelhp + '<BR/>';
                    html = html + '</td>';


                    html = html + '<td valign=\"top\" align=\"text-left\">';
                    html = html + data.patient[0].txt_pat_curradd1 + '<BR/>';
                    html = html + data.patient[0].txt_pat_curradd2 + '<BR/>';
                    html = html + data.patient[0].txt_pat_curradd3 + '<BR/>';
                    html = html + data.patient[0].txt_pat_currpostcode;
                    html = html + '</td>';

                    html = html + '<td valign=\"top\" align=\"left\">';
                    html = html + data.patient[0].txt_pat_offadd1 + '<BR/>';
                    html = html + data.patient[0].txt_pat_offadd2 + '<BR/>';
                    html = html + data.patient[0].txt_pat_offadd3 + '<BR/>';
                    html = html + data.patient[0].txt_pat_offpostcode;
                    html = html + '</td>';

                    html = html + '<td valign=\"top\" align=\"left\">';
                    html = html + data.patient[0].txt_pat_padd1 + '<BR/>';
                    html = html + data.patient[0].txt_pat_padd2 + '<BR/>';
                    html = html + data.patient[0].txt_pat_padd3 + '<BR/>';
                    html = html + data.patient[0].txt_pat_ppostcode;
                    html = html + '</td>';
	...
	$('#btn_register_patient').click(check_existing_patient);
