    ...
    if(istablet){
        ...
        $("#jqGrid").jqGrid({
            ...
            onSelectRow: function (rowid, selected){
                ...
                populate_labMain(selrowData('#jqGrid'));
                populate_ordcom_currpt(selrowData('#jqGrid'));
                populate_bloodtest(selrowData('#jqGrid'));
        ...
        $("#jqGrid").jqGrid({
            ...
            onSelectRow: function (rowid, selected){
                ...
                // empty_userfile();
                $('button#timer_stop').click();
                $('#labMain_tab .top.menu .item').tab('change tab','rehabilitation');
                // urlParam_trans.mrn = selrowData('#jqGrid').MRN;
                // urlParam_trans.episno = selrowData('#jqGrid').Episno;
                // urlParam_trans_diet.mrn = selrowData('#jqGrid').MRN;
                // urlParam_trans_diet.episno = selrowData('#jqGrid').Episno;
...
function populate_labMain(obj){
    // panel header
    $('#name_show_labMain').text(obj.Name);
    $('#mrn_show_labMain').text(("0000000" + obj.MRN).slice(-7));
    $('#sex_show_labMain').text(if_none(obj.Sex).toUpperCase());
    $('#dob_show_labMain').text(dob_chg(obj.DOB));
    $('#age_show_labMain').text(dob_age(obj.DOB)+' (YRS)');
    $('#race_show_labMain').text(if_none(obj.raceDesc).toUpperCase());
    $('#religion_show_labMain').text(if_none(obj.religion).toUpperCase());
    $('#occupation_show_labMain').text(if_none(obj.OccupCode).toUpperCase());
    $('#citizenship_show_labMain').text(if_none(obj.Citizencode).toUpperCase());
    $('#area_show_labMain').text(if_none(obj.AreaCode).toUpperCase());
   
    // formOccupTherapy
    $('#mrn_labMain').val(obj.MRN);
    $("#episno_labMain").val(obj.Episno);
    $("#age_labMain").val(dob_age(obj.DOB));
...
function stop_scroll_on(){
    $('div.paneldiv').on('mouseenter',function (){
        ...
        switch(parentdiv){
            ...
            default : SmoothScrollTo('#'+parentdiv, 300,75);break;
        }
