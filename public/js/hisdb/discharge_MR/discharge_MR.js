$(document).ready(function () {
	...
	$("#print_discharge").click(function() {
		window.open('./discharge_MR/showpdf?mrn_discharge='+$('#mrn_discharge').val()+'&episno_discharge='+$('#episno_discharge').val(), '_blank');
	});
	...
	$("#jqGrid_discharge_panel").on("shown.bs.collapse", function (){
		...
		$.post("discharge_MR/form?"+$.param(saveParam), $.param(postobj), function (data){
		...
		if( $('#form_discharge').isValid({requiredFields: ''}, conf, true) ) {
			...
			if (r == true) {
			  var postobj={	
				...
				$.post( "./discharge_MR/form", postobj, function( data ) {
...
button_state_discharge('empty');
	...
	function saveForm_discharge(callback){
		...
		$.post("./discharge_MR/form?"+$.param(saveParam), $.param(postobj)+'&'+$.param(values) , function (data){
