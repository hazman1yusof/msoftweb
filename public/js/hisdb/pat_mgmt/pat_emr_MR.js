...
$(document).ready(function () {
	...
	$("#jqGrid_nok_emr_MR").jqGrid({
		...
		height: 250, 
		rowNum: 30,
		pager: "#jqGridPager_nok_emr_MR",
		onSelectRow:function(rowid, selected){
			populate_nok_emr(selrowData("#jqGrid_nok_emr_MR"));
		},
		loadComplete: function(){
			$('#jqGrid_nok_emr_ilsave,#jqGrid_nok_emr_ilcancel').hide();

			let reccount = $('#jqGrid_nok_emr_MR').jqGrid('getGridParam', 'reccount');
			if(reccount>0){
				button_state_nok_emr('add_edit');
			}else{
				button_state_nok_emr('add');
			}
			$("#jqGrid_nok_emr_MR").setSelection($("#jqGrid_nok_emr_MR").getDataIDs()[0]);
	...
	$("#jqGrid_nok_emr_MR").inlineNav('#jqGridPager_nok_emr_MR', {edit:false,add:false,del:false,search:false,
		...
		onClickButton: function () {
			refreshGrid("#jqGrid_nok_emr_MR", urlParam_nok_emr);
		},
	});

	addParamField('#jqGrid_nok_emr_MR', false, urlParam_nok_emr);

	$("#tabNok_emr_MR").on("shown.bs.collapse", function(){
		$("#jqGrid_nok_emr_MR").jqGrid ('setGridWidth', Math.floor($("#jqGrid_nok_emr_c_MR")[0].offsetWidth-$("#jqGrid_nok_emr_c_MR")[0].offsetLeft-0));
	...
	var search_relate_emr = new ordialog(
		'search_relate_emr', 'hisdb.relationship', '#emr_relate_MR', 'errorField',
		{
			colModel: [
	...
	$("#add_nok_emr_MR").click(function(){
		button_state_nok_emr('wait');
		emptyFormdata_div('#form_nok_emr_MR');
		enableForm('#form_nok_emr_MR',['emr_relate_MR']);
		search_relate_emr.on();
		$("#save_nok_emr_MR").data('oper','add');
		
	});

	$("#edit_nok_emr_MR").click(function(){
		button_state_nok_emr('wait');
		enableForm('#form_nok_emr_MR',['emr_relate_MR']);
		search_relate_emr.on();
		$("#save_nok_emr_MR").data('oper','edit');
	});

	$("#save_nok_emr_MR").click(function(){
		disableForm('#form_nok_emr_MR');
		if( $('#form_nok_emr_MR').isValid({requiredFields: ''}, conf_nok, true) ) {
	...
	function saveForm_nok_emr(callback){
	    ...
	    var postobj={
	        oper:$("#save_nok_emr_MR").data('oper'),
	        idno:$("#emr_idno_MR").val(),
	    	_token : $('#csrf_token').val(),
	    	mrn : $("#pat_mrn").val(),
			name : $("#emr_name_MR").val(),
			relationshipcode : $("#emr_relate_MR").val(),
			tel_h : $("#emr_telh_MR").val(),
			tel_hp : $("#emr_telhp_MR").val(),
			email : $("#emr_email_MR").val()
	    };
	...
	$("#cancel_nok_emr_MR").click(function(){
	...
	function button_state_nok_emr(state){
		switch(state){
			case 'empty':
				$('#add_nok_emr_MR,#edit_nok_emr_MR,#save_nok_emr_MR,#cancel_nok_emr_MR').attr('disabled',true);
				break;
			case 'add_edit':
				$("#add_nok_emr_MR,#edit_nok_emr_MR").attr('disabled',false);
				$('#save_nok_emr_MR,#cancel_nok_emr_MR').attr('disabled',true);
				break;
			case 'add':
				$("#add_nok_emr_MR").attr('disabled',false);
				$('#edit_nok_emr_MR,#save_nok_emr_MR,#cancel_nok_emr_MR').attr('disabled',true);
				break;
			case 'wait':
				$("#save_nok_emr_MR,#cancel_nok_emr_MR").attr('disabled',false);
				$('#add_nok_emr_MR,#edit_nok_emr_MR').attr('disabled',true);
				break;
		}
	}

	function populate_nok_emr(obj){
		$("#emr_idno_MR").val(obj.idno);
		$("#emr_name_MR").val(obj.name);
		$("#emr_relate_MR").val(obj.relationship);
		$("#emr_telh_MR").val(obj.telh);
		$("#emr_telhp_MR").val(obj.telhp);
		$("#emr_email_MR").val(obj.email);
	}
