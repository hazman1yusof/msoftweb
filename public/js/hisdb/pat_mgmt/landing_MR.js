...
var grid = $("#grid-command-buttons").bootgrid({
        ...
        // url:undefined,
        url: "pat_mast_MR/post_entry?action=get_patient_list&epistycode="+$("#epistycode").val()+"&curpat="+$("#curpat").val()+"&PatClass="+$("#PatClass_MR").val()+'&mrn='+$('#mrnMR').val()+'&episno='+$('#episnoMR').val(),
        formatters: {
...
$(document).ready(function() {
    ...
    if($('#ismobile').val()=='true'){ // utk mobile
        ...
        $('#mdl_patient_header_MR').css('position','unset');
        $('#mdl_item_selector_MR div.modal-dialog').css('width','98%');
    }else{
    ...
    $( "#patientBox").click(function() { // register new patient
        ...
        if(oper_mykad == 'edit'){
            $("#toggle_tabNok_emr_MR,#toggle_tabNok_pat_MR").parent().show();
            $("#btn_register_patient").data("oper","edit");
        }else{
            $("#toggle_tabNok_emr_MR,#toggle_tabNok_pat_MR").parent().hide();
            $("#btn_register_patient").data("oper","add");
            $('#episno').val('1');
        }

        $('#mdl_patient_info_MR').modal({backdrop: "static"});
        $('#PatClass_MR').val(getUrlParameter('PatClass_MR'));
    });
    ...
    $('#mdl_accomodation_MR').on('show.bs.modal', function () {
    ...
    $.get( "./get_mykad_local?"+$.param(param), function( data ) {
        ...
        if(data.exists==true){
            if(data.pm_exists==true){
                ...
                $("#pat_mrn_MR").val(data.data.MRN);
                $("#txt_pat_idno_MR").val(data.data.idno);
                ...
                if($('#first_visit_date_MR').val() != ''){
                    $('#first_visit_date_MR').val(moment($('#first_visit_date_MR').val()).format('DD/MM/YYYY'));
                }

                if($('#last_visit_date_MR').val() != ''){
                    $('#last_visit_date_MR').val(moment($('#last_visit_date_MR').val()).format('DD/MM/YYYY'));
                }
...
function hide_all_panel(){
    $('div.panel-collapse:not(#tabBio_MR,#tabAddr_MR,#tabEpisode)').collapse('hide');
}
