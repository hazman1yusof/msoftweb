	...
	$("#jqGrid_rad").jqGrid({
		...
		colModel: [
			...
			{ label: 'Net<br>Amount', name: 'totamount', width: 60, align: 'right', classes: 'wrap txnum', editable:true,
				formatter:totamountFormatter_rad_doc,
				editrules:{required: true},editoptions:{readonly: "readonly"},
			},
		...
		loadComplete: function(data){
			calc_jq_height_onchange("jqGrid_rad",false,parseInt($('#jqGrid_ordcom_c').prop('clientHeight'))-160);
...
var myEditOptions_rad = {
	...
	oneditfunc: function (rowid) {
		...
		calc_jq_height_onchange("jqGrid_rad",true,parseInt($('#jqGrid_ordcom_c').prop('clientHeight'))-100);
	},
	...
	beforeSaveRow: function (options, rowid) {
		...
		let editurl = "./ordcom/form?"+
			$.param({
				...
				uom: $("#jqGrid_rad input[name='uom']").val(),
				uom_recv: $("#jqGrid_rad input[name='uom_recv']").val(),
...
var myEditOptions_rad_edit = {
	...
	oneditfunc: function (rowid) {
	    ...
	    $("#jqGrid_rad input[name='taxamount']").val(ret_parsefloat(selrowdata.taxamount));
	    $("#jqGrid_rad input[name='discamount']").val(ret_parsefloat(selrowdata.discamount));
	    $("#jqGrid_rad input[name='billtypeamt']").val(ret_parsefloat(selrowdata.billtypeamt));
	    $("#jqGrid_rad input[name='unitprce']").val(selrowdata.unitprce);
	    $("#jqGrid_rad input[name='uom']").val(selrowdata.uom);
	    $("#jqGrid_rad input[name='uom_recv']").val(selrowdata.uom_recv);

		// dialog_dosage_rad.on();
		// dialog_dosage_rad.id_optid = rowid;
		// dialog_dosage_rad.check(errorField,rowid+"_ftxtdosage","jqGrid_rad",null,null,null );

		// dialog_frequency_rad.on();
		// dialog_frequency_rad.id_optid = rowid;
		// dialog_frequency_rad.check(errorField,rowid+"_frequency","jqGrid_rad",null,null,null );

		// dialog_instruction_rad.on();
		// dialog_instruction_rad.id_optid = rowid;
		// dialog_instruction_rad.check(errorField,rowid+"_addinstruction","jqGrid_rad",null,null,null );

		// dialog_drugindicator_rad.on();
		// dialog_drugindicator_rad.id_optid = rowid;
		// dialog_drugindicator_rad.check(errorField,rowid+"_drugindicator","jqGrid_rad",null,null,null );
		...
		calc_jq_height_onchange("jqGrid_rad",true,parseInt($('#jqGrid_ordcom_c').prop('clientHeight'))-100);
		// $("#jqGrid_rad input[name='chgcode']").focus();
	},
	aftersavefunc: function (rowid, response, options) {
...
var dialog_chgcode_rad = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			dialog_chgcode_rad.urlParam.chgcode = data['chgcode'];

			$("#jqGrid_rad #"+id_optid+"_chgcode").val(data['chgcode']);
			$("#jqGrid_rad #"+id_optid+"_taxcode").val(data['taxcode']);
			$("#jqGrid_rad #"+id_optid+"_uom_rate").val(data['rate']);
		...
		open:function(obj_){
			...
			dialog_chgcode_rad.urlParam.url_chk = "./SalesOrderDetail/table";
			...
			dialog_chgcode_rad.urlParam.uom = $("#jqGrid_rad input[name='uom']").val();
		...
		close: function(obj){
			$("#jqGrid_rad input[name='uom_recv']").focus().select();
		}
...
var dialog_uomcode_rad = new ordialog(
	...
	{	colModel:
		...
		open:function(obj_){
			...
			dialog_uomcode_rad.urlParam.action_chk = "get_itemcode_uom_check";
...
var dialog_uom_recv_rad = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			if(data.invflag == '1' && (data.pt_idno == '' || data.pt_idno == null)){
				...
				myfail_msg_rad.add_fail({
					id:'nostock_rad',
					textfld:"#jqGrid_rad #"+id_optid+"_uom_recv",
					msg:'Item not available in store dept '+$("#jqGrid_rad #"+id_optid+"_deptcode").parent().next('span.help-block').text()+', please check',
				});
				...
				myfail_msg_rad.del_fail({id:'noprod_rad'});
				myfail_msg_rad.del_fail({id:'nostock_rad'});

				$("#jqGrid_rad #"+id_optid+"_convfactor_uom_recv").val(data['convfactor']);
				$("#jqGrid_rad #"+id_optid+"_qtyonhand").val(data['qtyonhand']);
				$("#jqGrid_rad #"+id_optid+"_quantity").val('');
				$("#jqGrid_rad #"+id_optid+"_uom_recv").val(data['uomcode']);
				$("#jqGrid_rad #"+id_optid+"_cost_price").val(data['avgcost']);
			}
		},
		gridComplete: function(obj){
			var gridname = '#'+obj.gridname;
		...
		title:"Select UOM Code For Item",
		open:function(obj_){
			dialog_uom_recv_rad.urlParam.url = "./SalesOrderDetail/table";
			dialog_uom_recv_rad.urlParam.action = 'get_itemcode_uom_recv';
			dialog_uom_recv_rad.urlParam.url_chk = "./SalesOrderDetail/table";
...
// var dialog_dosage_rad = new ordialog(
// 	'dosage_rad',['hisdb.dose'],"#jqGrid_rad input[name='dosage']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_dosage_rad.gridname);
// 			// $(dialog_dosage_rad.textfield).val(data.dosedesc);
// 			// $(dialog_dosage_rad.textfield+'_code').val(data.dosecode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_dosage_rad.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_dosage_rad.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_rad.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_dosage_rad.makedialog(false);

// var dialog_frequency_rad = new ordialog(
// 	'freq_rad',['hisdb.freq'],"#jqGrid_rad input[name='frequency']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_frequency_rad.gridname);
// 			// $(dialog_frequency_rad.textfield).val(data.freqdesc);
// 			// $(dialog_frequency_rad.textfield+'_code').val(data.freqcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_frequency_rad.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_frequency_rad.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_rad.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_frequency_rad.makedialog(false);

// var dialog_instruction_rad = new ordialog(
// 	'instruction_rad',['hisdb.instruction'],"#jqGrid_rad input[name='instruction']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_instruction_rad.gridname);
// 			// $(dialog_instruction_rad.textfield).val(data.description);
// 			// $(dialog_instruction_rad.textfield+'_code').val(data.inscode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_instruction_rad.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_instruction_rad.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_rad.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_instruction_rad.makedialog(false);

// var dialog_drugindicator_rad = new ordialog(
// 	'drugindicator_rad',['hisdb.drugindicator'],"#jqGrid_rad input[name='drugindicator']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_drugindicator_rad.gridname);
// 			// $(dialog_drugindicator_rad.textfield).val(data.description);
// 			// $(dialog_drugindicator_rad.textfield+'_code').val(data.drugindcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_drugindicator_rad.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_drugindicator_rad.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_rad.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_drugindicator_rad.makedialog(false);
...
function totamountFormatter_rad_doc(val,opt,rowObject ){
...
function dosageCustomEdit_rad(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="dosage" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function frequencyCustomEdit_rad(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="frequency" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function instructionCustomEdit_rad(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="instruction" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function drugindicatorCustomEdit_rad(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="drugindicator" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
