...
$(document).ready(function(){
	...
	$("#jqGrid_phys").jqGrid({
		...
		onSelectRow:function(rowid){
			...
			// write_detail_phys([
			// 	{span:'#jqgrid_detail_phys_chgcode',value:selrowdata.chgcode},
			// 	{span:'#jqgrid_detail_phys_chgcode_desc',value:selrowdata.chgcode},
			// 	{span:'#jqgrid_detail_phys_dept',value:selrowdata.deptcode},
			// 	{span:'#jqgrid_detail_phys_cost_price',value:selrowdata.cost_price},
			// 	{span:'#jqgrid_detail_phys_unitprice',value:selrowdata.unitprce},
			// 	{span:'#jqgrid_detail_phys_discamt',value:selrowdata.discamount},
			// 	{span:'#jqgrid_detail_phys_taxamt',value:selrowdata.taxamount},
			// ]);
	...
	jqgrid_physel_align_right("#jqGrid_phys");
...
var myEditOptions_phys = {
	...
	oneditfunc: function (rowid) {
		...
		$("#jqGrid_phys input[name='deptcode']").val($("#userdeptcode").val());
...
function calculate_line_totgst_and_totamt_phys(event) {
	...
	// write_detail_phys('#jqgrid_detail_phys_discamt',discamount);
...
var dialog_uomcode_phys = new ordialog(
	...
	{	colModel:
		...
		justb4refresh: function(obj_){
			dialog_uomcode_phys.urlParam.searchCol2=[];
			dialog_uomcode_phys.urlParam.searchVal2=[];
		},
...
var dialog_uom_recv_phys = new ordialog(
	...
	{	colModel:
		...
		justb4refresh: function(obj_){
			dialog_uom_recv_phys.urlParam.searchCol2=[];
			dialog_uom_recv_phys.urlParam.searchVal2=[];
		},
		justaftrefresh: function(obj_){
			$("#Dtext_"+obj_.unique).val('');
		}
...
function deptcodeCustomEdit_phys(val,opt){  	
	...
	if(val.trim() == ''){
		...
		// write_detail_phys('#jqgrid_detail_phys_dept',val);
	}
...
function remarkCustomEdit_phys(val,opt){
	var myreturn = `<label class='oe_phys_physel'>Dose</label><div class="oe_phys_div input-group"><input autocomplete="off" jqgrid="jqGrid_phys" optid="`+opt.id+`" id="`+opt.id+`" name="dosage" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label class='oe_phys_physel'>Frequency</label><div class="oe_phys_div input-group"><input autocomplete="off" jqgrid="jqGrid_phys" optid="`+opt.id+`" id="`+opt.id+`" name="frequency" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label class='oe_phys_physel'>Instruction</label><div class="oe_phys_div input-group"><input autocomplete="off" jqgrid="jqGrid_phys" optid="`+opt.id+`" id="`+opt.id+`" name="instruction" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label class='oe_phys_physel'>Indicator</label><div class="oe_phys_div input-group"><input autocomplete="off" jqgrid="jqGrid_phys" optid="`+opt.id+`" id="`+opt.id+`" name="drugindicator" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`
