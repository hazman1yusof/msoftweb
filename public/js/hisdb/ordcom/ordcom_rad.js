...
$(document).ready(function(){
	...
	$("#jqGrid_rad").jqGrid({
		...
		loadComplete: function(data){
			calc_jq_height_onchange("jqGrid_rad");
		},
...
var myEditOptions_rad = {
	...
	oneditfunc: function (rowid) {
		...
		Array.prototype.push.apply(mycurrency_rad.array, ["#jqGrid_rad input[name='unitprce']","#jqGrid_rad input[name='billtypeperct']","#jqGrid_rad input[name='billtypeamt']","#jqGrid_rad input[name='totamount']","#jqGrid_rad input[name='amount']","#jqGrid_rad input[name='taxamount']","#jqGrid_rad input[name='discamount']"]);
		Array.prototype.push.apply(mycurrency_np_rad.array, ["#jqGrid_rad input[name='qtyonhand']","#jqGrid_rad input[name='quantity']"]);
		...
		$("#jqGrid_rad input[name='unitprce'],#jqGrid_rad input[name='quantity']").on('keyup',{currency: [mycurrency_rad,mycurrency_np_rad]},calculate_line_totgst_and_totamt_rad);
		$("#jqGrid_rad input[name='unitprce'],#jqGrid_rad input[name='quantity']").on('blur',{currency: [mycurrency_rad,mycurrency_np_rad]},calculate_line_totgst_and_totamt_rad);
		// $("#jqGrid_rad input[name='unitprce'],#jqGrid_rad input[name='quantity']").on('blur',{currency: [mycurrency_rad,mycurrency_np_rad]},calculate_conversion_factor);

		// $("#jqGrid_rad input[name='qtyonhand']").keydown(function(e) {//when click tab at totamount, auto save
		// 	var code = e.keyCode || e.which;
		// 	if (code == '9'){
		// 		delay(function(){
		// 			$('#jqGrid_rad_ilsave').click();
		// 			addmore_jqGrid_rad.state = true;
		// 		}, 500 );
		// 	}
		// });

		calc_jq_height_onchange("jqGrid_rad",true);
...
var dialog_chgcode_rad = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('st_idno',data['st_idno']);
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('invflag',data['invflag']);
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('pt_idno',data['pt_idno']);
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('pt_idno',data['pt_idno']);
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('avgcost',data['avgcost']);
			$("#jqGrid_rad #"+id_optid+"_chgcode").data('convfactor',data['convfactor']);
			$('#'+dialog_chgcode_rad.gridname).data('fail_msg','');
			...
			// dialog_uomcode_rad.check(errorField);
			// dialog_uom_recv_rad.check(errorField);
			dialog_tax_rad.check(errorField);
			mycurrency_rad.formatOn();
...
function remarkCustomEdit_rad(val,opt){
	var myreturn = `<label>Dose</label><div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="dosage" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label>Frequency</label><div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="frequency" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label>Instruction</label><div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="instruction" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`;
	myreturn += `<label>Indicator</label><div class="input-group"><input autocomplete="off" jqgrid="jqGrid_rad" optid="`+opt.id+`" id="`+opt.id+`" name="drugindicator" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div>`
...
function cust_rules_rad(value, name) {
	...
	switch (name) {
		...
		case 'PO UOM': 
			temp = $("#jqGrid_rad input[name='pouom']"); 
		...
		case 'Tax Code': temp = $("#jqGrid_rad input[name='taxcode']"); break;
