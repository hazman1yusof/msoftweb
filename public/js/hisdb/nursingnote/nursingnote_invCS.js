...
$(document).ready(function(){
	...
	/////////////////////////////////myEditOptions/////////////////////////////////
	var myEditOptions_addInvChartCS = {
		keys: true,
		extraparam: {
			"_token": $("#csrf_token").val()
		},
		oneditfunc: function (rowid){
			$("#jqGridPagerDelete_addnotesInvChartCS,#jqGridPagerRefresh_addnoteInvChartCS").hide();
			
			$("textarea[name='note']").keydown(function (e){ // when click tab at last column in header, auto save
				var code = e.keyCode || e.which;
				if (code == '9')$('#jqGridAddNotesInvChartCS_ilsave').click();
				// addmore_jqgridInvChartCS.state = true;
				// $('#jqGrid_ilsave').click();
			});
		},
		aftersavefunc: function (rowid, response, options){
			// addmore_jqgridInvChartCS.more = true; // only addmore after save inline
			// state true maksudnyer ada isi, tak kosong
			refreshGrid('#jqGridAddNotesInvChartCS',urlParam_AddNotesInvChartCS,'add_notesInvChartCS');
			errorField.length = 0;
			$("#jqGridPagerDelete_addnotesInvChartCS,#jqGridPagerRefresh_addnoteInvChartCS").show();
		},
		...
		beforeSaveRow: function (options, rowid){
			...
			let data = $('#jqGridAddNotesInvChartCS').jqGrid ('getRowData', rowid);
			
			let editurl = "./nursingnote/form?"+
				$.param({
					episno: $('#episno_nursNote').val(),
					mrn: $('#mrn_nursNote').val(),
					action: 'addNotesInvChartCS_save',
				});
			$("#jqGridAddNotesInvChartCS").jqGrid('setGridParam', { editurl: editurl });
		},
	...
	$("#jqGridAddNotesInvChartCS").inlineNav('#jqGridPagerAddNotesInvChartCS', {
		...
		restoreAfterSelect: false,
		addParams: {
			addRowParams: myEditOptions_addInvChartCS
		},
