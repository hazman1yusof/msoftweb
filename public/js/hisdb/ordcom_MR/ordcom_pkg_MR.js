	...
	$("#jqGrid_pkg").jqGrid({
		...
		subGridRowExpanded: function(subgrid_id, row_id) {
			...
			$("#"+subgrid_table_id).jqGrid({
				url:"./ordcom_MR/table?action=ordcom_table_pkgdet&id="+selrowdata.id,
				datatype: "json",
				colModel: [
...
function calculate_line_totgst_and_totamt_pkg(event) {
	...
	var billty_discamt = ret_parsefloat($("#jqGrid_pkg #"+id_optid+"_chgcode").data('billty_discamt')).toFixed(2);
...
var dialog_chgcode_pkg = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			$("#jqGrid_pkg #"+id_optid+"_chgcode").data('billty_discamt',data['billty_discamt']);
		...
		open:function(obj_){
			...
			dialog_chgcode_pkg.urlParam.url_chk = "./ordcom_MR/table";
...
// 		title:"Select UOM Code For Item",
// 		open:function(obj_){
// 			dialog_uom_recv_pkg.urlParam.url = "./ordcom_MR/table";
// 			dialog_uom_recv_pkg.urlParam.action = 'get_itemcode_uom_recv';
// 			dialog_uom_recv_pkg.urlParam.url_chk = "./ordcom_MR/table";
