	...
	$("#jqGrid_diet").jqGrid({
		...
		colModel: [
			...
			{ label: 'Net<br>Amount', name: 'totamount', width: 60, align: 'right', classes: 'wrap txnum', editable:true,
				formatter:totamountFormatter_diet_doc,
				editrules:{required: true},editoptions:{readonly: "readonly"},
			},
	...
	beforeSaveRow: function (options, rowid) {
		...
		let editurl = "./ordcom/form?"+
			$.param({
				...
				uom_recv: $("#jqGrid_diet input[name='uom_recv']").val(),
...
var myEditOptions_diet_edit = {
	...
	oneditfunc: function (rowid) {
	    ...
	    $("#jqGrid_diet input[name='taxamount']").val(ret_parsefloat(selrowdata.taxamount));
	    $("#jqGrid_diet input[name='discamount']").val(ret_parsefloat(selrowdata.discamount));
	    $("#jqGrid_diet input[name='billtypeamt']").val(ret_parsefloat(selrowdata.billtypeamt));
	    $("#jqGrid_diet input[name='unitprce']").val(selrowdata.unitprce);
	    $("#jqGrid_diet input[name='uom']").val(selrowdata.uom);
	    $("#jqGrid_diet input[name='uom_recv']").val(selrowdata.uom_recv);

		// dialog_dosage_diet.on();
		// dialog_dosage_diet.id_optid = rowid;
		// dialog_dosage_diet.check(errorField,rowid+"_ftxtdosage","jqGrid_diet",null,null,null );

		// dialog_frequency_diet.on();
		// dialog_frequency_diet.id_optid = rowid;
		// dialog_frequency_diet.check(errorField,rowid+"_frequency","jqGrid_diet",null,null,null );

		// dialog_instruction_diet.on();
		// dialog_instruction_diet.id_optid = rowid;
		// dialog_instruction_diet.check(errorField,rowid+"_addinstruction","jqGrid_diet",null,null,null );

		// dialog_drugindicator_diet.on();
		// dialog_drugindicator_diet.id_optid = rowid;
		// dialog_drugindicator_diet.check(errorField,rowid+"_drugindicator","jqGrid_diet",null,null,null );
...
var dialog_chgcode_diet = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			dialog_chgcode_diet.urlParam.chgcode = data['chgcode'];
		...
		open:function(obj_){
			...
			dialog_chgcode_diet.urlParam.uom = $("#jqGrid_diet input[name='uom']").val();
		...
		close: function(obj){
			$("#jqGrid_diet input[name='uom_recv']").focus().select();
		}
...
var dialog_uomcode_diet = new ordialog(
	...
	{	colModel:
		...
		open:function(obj_){
			...
			dialog_uomcode_diet.urlParam.action_chk = "get_itemcode_uom_check";
...
var dialog_uom_recv_diet = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			if(data.invflag == '1' && (data.pt_idno == '' || data.pt_idno == null)){
				...
				myfail_msg_diet.add_fail({
					id:'nostock_diet',
					textfld:"#jqGrid_diet #"+id_optid+"_uom_recv",
					msg:'Item not available in store dept '+$("#jqGrid_diet #"+id_optid+"_deptcode").parent().next('span.help-block').text()+', please check',
				});
				...
				myfail_msg_diet.del_fail({id:'noprod_diet'});
				myfail_msg_diet.del_fail({id:'nostock_diet'});
		...
		title:"Select UOM Code For Item",
		open:function(obj_){
			dialog_uom_recv_diet.urlParam.url = "./SalesOrderDetail/table";
			dialog_uom_recv_diet.urlParam.action = 'get_itemcode_uom_recv';
			dialog_uom_recv_diet.urlParam.url_chk = "./SalesOrderDetail/table";
...
// var dialog_dosage_diet = new ordialog(
// 	'dosage_diet',['hisdb.dose'],"#jqGrid_diet input[name='dosage']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_dosage_diet.gridname);
// 			// $(dialog_dosage_diet.textfield).val(data.dosedesc);
// 			// $(dialog_dosage_diet.textfield+'_code').val(data.dosecode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_dosage_diet.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_dosage_diet.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_diet.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_dosage_diet.makedialog(false);

// var dialog_frequency_diet = new ordialog(
// 	'freq_diet',['hisdb.freq'],"#jqGrid_diet input[name='frequency']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_frequency_diet.gridname);
// 			// $(dialog_frequency_diet.textfield).val(data.freqdesc);
// 			// $(dialog_frequency_diet.textfield+'_code').val(data.freqcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_frequency_diet.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_frequency_diet.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_diet.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_frequency_diet.makedialog(false);

// var dialog_instruction_diet = new ordialog(
// 	'instruction_diet',['hisdb.instruction'],"#jqGrid_diet input[name='instruction']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_instruction_diet.gridname);
// 			// $(dialog_instruction_diet.textfield).val(data.description);
// 			// $(dialog_instruction_diet.textfield+'_code').val(data.inscode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_instruction_diet.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_instruction_diet.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_diet.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_instruction_diet.makedialog(false);

// var dialog_drugindicator_diet = new ordialog(
// 	'drugindicator_diet',['hisdb.drugindicator'],"#jqGrid_diet input[name='drugindicator']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_drugindicator_diet.gridname);
// 			// $(dialog_drugindicator_diet.textfield).val(data.description);
// 			// $(dialog_drugindicator_diet.textfield+'_code').val(data.drugindcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_drugindicator_diet.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_drugindicator_diet.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_diet.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_drugindicator_diet.makedialog(false);
...
function totamountFormatter_diet_doc(val,opt,rowObject ){
...
function dosageCustomEdit_diet(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_diet" optid="`+opt.id+`" id="`+opt.id+`" name="dosage" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function frequencyCustomEdit_diet(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_diet" optid="`+opt.id+`" id="`+opt.id+`" name="frequency" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function instructionCustomEdit_diet(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_diet" optid="`+opt.id+`" id="`+opt.id+`" name="instruction" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function drugindicatorCustomEdit_diet(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
