	...
	$("#jqGrid_oth").jqGrid({
		...
		colModel: [
			...
			{ label: 'Net<br>Amount', name: 'totamount', width: 60, align: 'right', classes: 'wrap txnum', editable:true,
				formatter:totamountFormatter_oth_doc,
				editrules:{required: true},editoptions:{readonly: "readonly"},
			},
		...
		loadComplete: function(data){
			calc_jq_height_onchange("jqGrid_oth",false,parseInt($('#jqGrid_ordcom_c').prop('clientHeight'))-160);
...
var myEditOptions_oth = {
	...
	oneditfunc: function (rowid) {
		...
		calc_jq_height_onchange("jqGrid_oth",true,parseInt($('#jqGrid_ordcom_c').prop('clientHeight'))-100);
	},
	...
	beforeSaveRow: function (options, rowid) {
		...
		let editurl = "./ordcom/form?"+
			$.param({
				...
				uom_recv: $("#jqGrid_oth input[name='uom_recv']").val(),
	...
	oneditfunc: function (rowid) {
		...
		dialog_chgcode_oth.check(errorField,rowid+"_chgcode","jqGrid_oth",null,
        	function(self){
				...
				self.urlParam.filterVal = [$('#ordcomtt_oth').val()];
	    ...
	    $("#jqGrid_oth input[name='taxamount']").val(ret_parsefloat(selrowdata.taxamount));
	    $("#jqGrid_oth input[name='discamount']").val(ret_parsefloat(selrowdata.discamount));
	    $("#jqGrid_oth input[name='billtypeamt']").val(ret_parsefloat(selrowdata.billtypeamt));
	    $("#jqGrid_oth input[name='unitprce']").val(selrowdata.unitprce);
	    $("#jqGrid_oth input[name='uom']").val(selrowdata.uom);
	    $("#jqGrid_oth input[name='uom_recv']").val(selrowdata.uom_recv);

		// dialog_dosage_oth.on();
		// dialog_dosage_oth.id_optid = rowid;
		// dialog_dosage_oth.check(errorField,rowid+"_ftxtdosage","jqGrid_oth",null,null,null );

		// dialog_frequency_oth.on();
		// dialog_frequency_oth.id_optid = rowid;
		// dialog_frequency_oth.check(errorField,rowid+"_frequency","jqGrid_oth",null,null,null );

		// dialog_instruction_oth.on();
		// dialog_instruction_oth.id_optid = rowid;
		// dialog_instruction_oth.check(errorField,rowid+"_addinstruction","jqGrid_oth",null,null,null );

		// dialog_drugindicator_oth.on();
		// dialog_drugindicator_oth.id_optid = rowid;
		// dialog_drugindicator_oth.check(errorField,rowid+"_drugindicator","jqGrid_oth",null,null,null );
...
var dialog_chgcode_oth = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			dialog_chgcode_oth.urlParam.chgcode = data['chgcode'];
		...
		open:function(obj_){
			...
			dialog_chgcode_oth.urlParam.uom = $("#jqGrid_oth input[name='uom']").val();
		...
		close: function(obj){
			$("#jqGrid_oth input[name='uom_recv']").focus().select();
		}
...
var dialog_uomcode_oth = new ordialog(
	...
	{	colModel:
		...
		open:function(obj_){
			...
			dialog_uomcode_oth.urlParam.action_chk = "get_itemcode_uom_check";
			...
			dialog_uomcode_oth.urlParam.filterCol = ['cm.chggroup'];
			dialog_uomcode_oth.urlParam.filterVal = [$('#ordcomtt_oth').val()];
		},
		close: function(){
			$("#jqGrid_oth input[name='uom_recv']").focus().select();
...
var dialog_uom_recv_oth = new ordialog(
	...
	{	colModel:
		...
		ondblClickRow:function(event){
			...
			if(data.invflag == '1' && (data.pt_idno == '' || data.pt_idno == null)){
				...
				myfail_msg_oth.add_fail({
					id:'nostock_oth',
					textfld:"#jqGrid_oth #"+id_optid+"_uom_recv",
					msg:'Item not available in store dept '+$("#jqGrid_oth #"+id_optid+"_deptcode").parent().next('span.help-block').text()+', please check',
				});
				...
				myfail_msg_oth.del_fail({id:'noprod_oth'});
				myfail_msg_oth.del_fail({id:'nostock_oth'});
		...
		title:"Select UOM Code For Item",
		open:function(obj_){
			dialog_uom_recv_oth.urlParam.url = "./SalesOrderDetail/table";
			dialog_uom_recv_oth.urlParam.action = 'get_itemcode_uom_recv';
			dialog_uom_recv_oth.urlParam.url_chk = "./SalesOrderDetail/table";
			...
			dialog_uom_recv_oth.urlParam.filterCol = ['cm.chggroup'];
			dialog_uom_recv_oth.urlParam.filterVal = [$('#ordcomtt_oth').val()];
		},
		close: function(){
			$("#jqGrid_oth input[name='quantity']").focus().select();
			// $(dialog_uomcode_oth.textfield)			//lepas close dialog focus on next textfield 
...
// var dialog_dosage_oth = new ordialog(
// 	'dosage_oth',['hisdb.dose'],"#jqGrid_oth input[name='dosage']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_dosage_oth.gridname);
// 			// $(dialog_dosage_oth.textfield).val(data.dosedesc);
// 			// $(dialog_dosage_oth.textfield+'_code').val(data.dosecode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_dosage_oth.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_dosage_oth.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_oth.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_dosage_oth.makedialog(false);

// var dialog_frequency_oth = new ordialog(
// 	'freq_oth',['hisdb.freq'],"#jqGrid_oth input[name='frequency']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_frequency_oth.gridname);
// 			// $(dialog_frequency_oth.textfield).val(data.freqdesc);
// 			// $(dialog_frequency_oth.textfield+'_code').val(data.freqcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_frequency_oth.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_frequency_oth.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_oth.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_frequency_oth.makedialog(false);

// var dialog_instruction_oth = new ordialog(
// 	'instruction_oth',['hisdb.instruction'],"#jqGrid_oth input[name='instruction']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_instruction_oth.gridname);
// 			// $(dialog_instruction_oth.textfield).val(data.description);
// 			// $(dialog_instruction_oth.textfield+'_code').val(data.inscode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_instruction_oth.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_instruction_oth.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_oth.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_instruction_oth.makedialog(false);

// var dialog_drugindicator_oth = new ordialog(
// 	'drugindicator_oth',['hisdb.drugindicator'],"#jqGrid_oth input[name='drugindicator']",errorField,
// 	{	colModel:
// 		[
...
// 				},
// 		ondblClickRow:function(event){
// 			let data=selrowData('#'+dialog_drugindicator_oth.gridname);
// 			// $(dialog_drugindicator_oth.textfield).val(data.description);
// 			// $(dialog_drugindicator_oth.textfield+'_code').val(data.drugindcode);
// 		},
// 		gridComplete: function(obj){
// 			var gridname = '#'+obj.gridname;
// 			if($(gridname).jqGrid('getDataIDs').length == 1 && obj.ontabbing == true){
...
// 		title:"Select Tax Code For Item",
// 		open:function(obj_){
// 			dialog_drugindicator_oth.urlParam.filterCol=['compcode','recstatus'];
// 			dialog_drugindicator_oth.urlParam.filterVal=['session.compcode','ACTIVE'];
// 		},
// 		close: function(){
// 			// $(dialog_deptcode_oth.textfield)			//lepas close dialog focus on next textfield 
// 			// 	.closest('td')						//utk dialog dalam jqgrid jer
...
// 	},'urlParam', 'radio', 'tab' 	
// );
// dialog_drugindicator_oth.makedialog(false);
...
function totamountFormatter_oth_doc(val,opt,rowObject ){
...
function dosageCustomEdit_oth(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_oth" optid="`+opt.id+`" id="`+opt.id+`" name="dosage" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function frequencyCustomEdit_oth(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_oth" optid="`+opt.id+`" id="`+opt.id+`" name="frequency" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function instructionCustomEdit_oth(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
	return $(`<div class="input-group"><input autocomplete="off" jqgrid="jqGrid_oth" optid="`+opt.id+`" id="`+opt.id+`" name="instruction" type="text" class="form-control input-sm" style="text-transform:uppercase" data-validation="required" value="`+val+`" style="z-index: 0"><a class="input-group-addon btn btn-primary"><span class="fa fa-ellipsis-h"></span></a></div><span class="help-block"></span>`);
}
function drugindicatorCustomEdit_oth(val,opt){
	val = (val.slice(0, val.search("[<]")) == "undefined") ? "" : val.slice(0, val.search("[<]"));	
