...
$(document).ready(function () {
	...
	var urlParam={
		...
		table_id:'idno',
		sort_idno:true,
		filterCol:['compcode','TYPE'],
		filterVal:['session.compcode','OT']
	}
	...
	var myEditOptions = {
		...
		beforeSaveRow: function (options, rowid) {
			...
			let editurl = "./ot_maintenance/form?"+
				$.param({
					action: 'ot_maintenance_save',
				});
	...
	var myEditOptions_edit = {
		...
		oneditfunc: function (rowid) {
			$("#jqGridPagerDelete,#jqGridPagerRefresh").hide();
			$("input[name='resourcecode']").attr('disabled','disabled');
			$("input[name='description']").keydown(function(e) {//when click tab at last column in header, auto save
				var code = e.keyCode || e.which;
				if (code == '9')$('#jqGrid_ilsave').click();
				/*addmore_jqgrid.state = true;
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if (result == true) {
							...
							$.post( "./ot_maintenance/form?"+$.param(param),{oper:'del'}, function( data ){
							}).fail(function (data) {
