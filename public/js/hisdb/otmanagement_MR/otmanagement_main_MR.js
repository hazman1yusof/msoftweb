...
var urlParam_OT = {
	...
	url: "otmanagement_MR/table?action=get_table_operRecList&epistycode="+$("#epistycode").val()+"&curpat="+$("#curpat").val()+"&PatClass="+$("#PatClass_MR").val()+'&mrn='+$('#mrnMR').val()+'&episno='+$('#episnoMR').val(),
...
$(document).ready(function () {
	...
	$('#calendar').fullCalendar({
		...
		dayClick: function(date, allDay, jsEvent, view) {
			...
			urlParam_OT.filterVal[0] = date.format('YYYY-MM-DD');
		...
		eventClick: function(event) {
			...
			if(view.type == 'listMonth'){
				urlParam_OT.filterVal[0] = event.start.format('YYYY-MM-DD');
				refreshGrid("#jqGrid_OT", urlParam_OT);
			}
		},
		eventSources: [
	...
	if(istablet){
		...
		$('#jqgrid_OT_div').removeClass('eleven wide tablet eleven wide computer');
		$('#jqgrid_OT_div').addClass('sixteen wide tablet sixteen wide computer');
		...
		$('#button_calendar').calendar({
			...
			onSelect: function(date,mode){
				...
				urlParam_OT.filterVal[0] = new_date;
		...
		$("#jqGrid_OT").jqGrid({
			...
			onSelectRow:function(rowid, selected){
				...
				populate_otMain(selrowData('#jqGrid_OT'));
				populate_otmgmt_div(selrowData('#jqGrid_OT'));
				populate_preoperative(selrowData('#jqGrid_OT'));
				populate_preoperativeDC(selrowData('#jqGrid_OT'));
				populate_oper_team(selrowData('#jqGrid_OT'));
				populate_otswab(selrowData('#jqGrid_OT'));
				populate_ottime(selrowData('#jqGrid_OT'));
				populate_otdischarge(selrowData('#jqGrid_OT'));
				populate_endoscopyNotes(selrowData('#jqGrid_OT'));
			...
			gridComplete: function () {
				...
				$('#no_of_pat').text($("#jqGrid_OT").getGridParam("reccount"));
				empty_otMain();
				empty_otmgmt_div();
				empty_preoperative();
			...
			loadComplete: function (){
				...
				var selectedRowId = $("#jqGrid_OT").jqGrid('getGridParam', 'selrow');
	...
	function ordercompleteInit(){
		$('input[type=checkbox].checkbox_completed').on('change',function(e){
			...
			if(cont ==  false){
				...
				let rowdata = $('#jqGrid_OT').jqGrid ('getRowData', rowid);
...
function init_editbtn_top(){
	...
	$('button.savebuttop').on('click',function(e){
		...
		$.post( "./otmanagement/form?action=edit_header_ot",obj, function( data ){
			...
			refreshGrid("#jqGrid_OT", urlParam_OT,'edit');
		}).done(function(data){
			refreshGrid("#jqGrid_OT", urlParam_OT,'edit');
		});
