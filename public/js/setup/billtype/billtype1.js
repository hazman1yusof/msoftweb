		...
		$(document).ready(function () {
			...
			$("#jqGridsvc").jqGrid({
				datatype: "local",
				 colModel: [
					...
					{label: 'Chg. Group', name: 'svc_chggroup', width: 90,canSearch: true, checked:true},
					...
					{label: 'Percentage', name: 'svc_percent_', width: 90,formatter:formatter1,unformat:unformat1},
					{label: 'All Item', name: 'svc_allitem', width: 90, formatter:formatter,unformat:unformat},
					{label: 'Discount Charge Code', name: 'svc_discchgcode', width: 90},
					{label: 'discrate', name: 'svc_discrate', width: 90 , hidden: true},
					{label: 'recstatus', name: 'svc_recstatus', width: 90 , hidden: true},
				],
				viewrecords: true,
				autowidth:true,
                multiSort: true,
				...
				onSelectRow:function(rowid, selected){
					if(rowid != null) {
						...
						$("#Fitem :input[name*='svc_billtype']").val(rowData['svc_billtype']);
						$("#Fitem :input[name*='svc_chggroup']").val(rowData['svc_chggroup']);
						$("#Fitem :input[name*='cc_description']").val(rowData['cc_description']);
			...
			$("#jqGridsvc").jqGrid('navGrid','#jqGridPager2',{	
				...
				onClickButton: function(){
					...
					var billtype = $("#jqGridsvc").jqGrid ('getGridParam', 'selrow');
					if(!selRowId){
						alert('Please select row');
						return emptyFormdata(errorField,'#Fsvc');
					}else{
						saveFormdata("#jqGridsvc","#Dsvc","#Fsvc",'del',saveParam,{'chggroup':selRowId});
					}
				}, 
				position: "first", 
				title:"Delete Selected Row", 
			...
			dialog_chgcode=new makeDialog('hisdb.chgmast',"#Fitem :input[name*='chgcode']",['chgcode','description'], 'Chg. Code');
			...
			$("#jqGriditem").jqGrid({
				datatype: "local",
				 colModel: [
					...
					{label: 'Chg. Group', name: 'i_chggroup', width: 90,},
					{label: 'Description', name: 'c_description', width: 90},
					{label: 'Chg Code', name: 'i_chgcode', width: 90, canSearch: true, checked:true},
					{label: 'Price', name: 'i_price', width: 90, canSearch: true },
					{label: 'Amount', name: 'i_amount', width: 90 },
					{label: 'discrate', name: 'i_discrate', width: 90 , hidden: true},
					{label: 'recstatus', name: 'i_recstatus', width: 90 , hidden: true},
					{label: 'Percentage', name: 'i_percent_', width: 90,formatter:formatter1,unformat:unformat1},
				],
				viewrecords: true,
				autowidth:true,
                multiSort: true,
			...
			$("#jqGriditem").jqGrid('navGrid','#jqGridPager3',{	
				...
				onClickButton: function(){
					...
					//var billtype = $("#Fitem :input[name='i.billtype']").val(rowData['svc.billtype']);
				...
				onClickButton: function(rowid, selected){
					...
					$( "#Ditem" ).dialog( "open" );
					//var xx= 
					$("#Fitem :input[name*='billtype']").val(selrowData('#jqGrid').billtype);
					$("#Fitem :input[name*='chggroup']").val(selrowData('#jqGridsvc').chggroup);
					/*$("#Fitem :input[name*='i.billtype']").val(rowData['svc.billtype']);
					$("#Fitem :input[name*='i.chggroup']").val(rowData['chggroup']);
					$("#Fitem :input[name*='description']").val(rowData['cc.description']);*/
					$("#Fitem :input[name*='description']").val(selrowData('#jqGridsvc').description);

					//$("#Fitem :input[name*='c.description']").val(selrowData('#jqGriditem').description);
					//$("#Fitem :input[name*='i.billtype']").val(selrowData('#jqGridsvc').svc[billtype]);
					/*selrow = $('#jqGriditem').jqGrid ('getGridParam', 'selrow');
					data = $('#jqGriditem').jqGrid ('getRowData', selrow);*/
					//console.log(selrow['cc.description']);
					//console.log(data['i.chggroup']);
				}, 
				position: "first", 
				title:"Add New Row", 
				cursor: "pointer"
			...
			/*jQuery("#jqGriditem").jqGrid('setGroupHeaders', {
			  useColSpanStyle: false, 
			  groupHeaders:[
				{startColumnName: 'i.chggroup', numberOfColumns: 7, titleText: 'Bill Type Item'},
			  ]
			});*/

			addParamField('#jqGriditem',false,urlParam_item);
			addParamField('#jqGriditem',false,saveParam_item,["c.description"]);
			...
			$( "#dialog" ).dialog({
				...
				open: function(){
					...
					if(selText=="#Fitem :input[name*='chgcode']"){
