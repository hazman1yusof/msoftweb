...
$(document).ready(function () {
	...
	$("#jqGrid").jqGrid({
		...
		colModel: [
			...
			{ label: 'Code', name: 'bloodcode', width: 50, classes: 'wrap', canSearch: true, editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase"}},
	...
	var myEditOptions_edit = {
		...
		oneditfunc: function (rowid) {
			$("#jqGridPagerDelete,#jqGridPagerRefresh").hide();
			$("input[name='bloodcode']").attr('disabled','disabled');
			$("input[name='Description']").keydown(function(e) {//when click tab at last column in header, auto save
				var code = e.keyCode || e.which;
				if (code == '9')$('#jqGrid_ilsave').click();
				/*addmore_jqgrid.state = true;
