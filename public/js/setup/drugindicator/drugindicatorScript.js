...
$(document).ready(function () {
    ...
    var err_reroll = new err_reroll('#jqGrid',['drugindcode', 'description']);
    ...
    $("#jqGrid").jqGrid({
        datatype: "local",
        editurl: "./drugindicator/form",
         colModel: [
            {label: 'Compcode', name: 'compcode', width: 90 , hidden: true},
            { label: 'idno', name: 'idno', width: 5,hidden:true, key:true},
            { label: 'Drug Indicator Code', name: 'drugindcode', width: 50, classes: 'wrap', canSearch: true,editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase"}},
            { label: 'Description', name: 'description', classes: 'wrap', canSearch: true, width: 100, checked:true,editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase"}},
            ...
            { label: 'Status', name: 'recstatus', width: 50, classes: 'wrap', hidden: false, editable: true, edittype:"select",formatter:'select', editoptions:{value:"ACTIVE:ACTIVE;DEACTIVE:DEACTIVE"}, 
		...
		var chk = ['drugindcode','description'];
    ...
    var myEditOptions = {
        ...
        beforeSaveRow: function (options, rowid) {
            ...
            let editurl = "./drugindicator/form?"+
                $.param({
                    action: 'drugindicator_save',
                });
    ...
    var myEditOptions_edit = {
        ...
        oneditfunc: function (rowid) {
            $("#jqGridPagerDelete,#jqGridPagerRefresh").hide();
            $("#description").focus().select();
            $("input[name='drugindcode']").attr('disabled','disabled');
            $("input[name='description']").keydown(function(e) {//when click tab at last column in header, auto save
                var code = e.keyCode || e.which;
                if (code == '9')$('#jqGrid_ilsave').click();
                /*addmore_jqgrid.state = true;
                    ...
                    callback: function (result) {
                        if (result == true) {
                            param = {
                                _token: $("#_token").val(),
                                action: 'drugindicator_save',
                                drugindcode: $('#drugindcode').val(),
                                idno: selrowData('#jqGrid').idno,
                            }
                            $.post( "./drugindicator/form?"+$.param(param),{oper:'del'}, function( data ){
                            }).fail(function (data) {
