...
$(document).ready(function () {
	...
	var err_reroll = new err_reroll('#jqGrid',['deptcode', 'description']);
	...
	$("#jqGrid").jqGrid({
		...
		colModel: [
			...
			{ label: 'Ward', name: 'deptcode', width: 30, canSearch: true, checked: true, editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase" }},
			...
			//{ label: 'Unit', name: 'sector', width: 15, canSearch: true, checked: true, editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase" }},
			//{ label: 'Region', name: 'region', width: 15, canSearch: true, checked: true, editable: true, editrules: { required: true }, editoptions: {style: "text-transform: uppercase" }},
			...
			{ label: 'Last User', name: 'upduser', width: 50, hidden: false },
	...
	var myEditOptions_edit = {
		...
		oneditfunc: function (rowid) {
			$("#jqGridPagerDelete,#jqGridPagerRefresh").hide();
			$("input[name='warddept']").attr('disabled','disabled');
			$("select[name='recstatus']").keydown(function(e) {//when click tab at last column in header, auto save
				var code = e.keyCode || e.which;
				if (code == '9')$('#jqGrid_ilsave').click();
				/*addmore_jqgrid.state = true;
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if (result == true) {
							...
							$.post( "/ward/form?"+$.param(param),{oper:'del'}, function( data ){
							}).fail(function (data) {
