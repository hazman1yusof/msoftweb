...
$(document).ready(function (){
	...
	$("#save_doctorNote").click(function (){
		if($('#formDoctorNote').isValid({requiredFields: ''}, conf, true)){
			saveForm_doctorNote(function (data){
				emptyFormdata_div("#formDoctorNote",['#mrn_doctorNote','#episno_doctorNote']);
				disableForm('#formDoctorNote',['toggle_type']);
				docnote_date_tbl.ajax.url("./ptcare_doctornote_MR/table?"+$.param(dateParam_docnote)).load(function (){
	...
	$("#jqGridAddNotes").jqGrid({
		datatype: "local",
		editurl: "./ptcare_doctornote_MR/form",
		colModel: [
	...
	var myEditOptions_add = {
		...
		beforeSaveRow: function (options, rowid){
			...
			let editurl = "./ptcare_doctornote_MR/form?"+
				$.param({
					_token: $('#_token').val(),
	...
	// 				$.post("./ptcare_doctornote_MR/form?"+$.param(param), {oper:'del'}, function (data){
...
var dialog_icd = new ordialog(
	'icdcode',['hisdb.diagtab AS dt','sysdb.sysparam AS sp'],"#formDoctorNote input[name='icdcode']",errorField,
	{
		...
		urlParam: {
			url: "ptcare_doctornote_MR/table",
		...
		title: "Select ICD",
		open: function (){
			dialog_icd.urlParam.url = "ptcare_doctornote_MR/table";
...
function saveForm_doctorNote(callback){
	...
	$.post("./ptcare_doctornote_MR/form?"+$.param(saveParam), $.param(postobj)+'&'+$.param(values), function (data){
...
$('#tab_doctornote').on('shown.bs.collapse', function (){
	...
	docnote_date_tbl.ajax.url("./ptcare_doctornote_MR/table?"+$.param(dateParam_docnote)).load(function (data){
		emptyFormdata_div("#formDoctorNote",['#mrn_doctorNote','#episno_doctorNote']);
		// $('#docnote_date_tbl tbody tr:eq(0)').click(); // to select first row
	});
...
$('#docnote_date_tbl tbody').on('click', 'tr', function (){
	...
	$.get("./ptcare_doctornote_MR/table?"+$.param(doctornote_docnote), function (data){
