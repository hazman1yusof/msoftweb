...
$(document).ready(function (){
	...
	$("#save_doctorNote").click(function (){
		if($('#formDoctorNote').isValid({requiredFields: ''}, conf, true)){
			saveForm_doctorNote(function (data){
				emptyFormdata_div("#formDoctorNote",['#mrn_emergencyMain','#episno_emergencyMain']);
				disableForm('#formDoctorNote',['toggle_type']);
				docnote_date_tbl.ajax.url("./ptcare_doctornoteED_MR/table?"+$.param(dateParam_docnote)).load(function (){
	...
	$("#jqGridAddNotes").jqGrid({
		datatype: "local",
		editurl: "./ptcare_doctornoteED_MR/form",
		colModel: [
	...
	var myEditOptions_add = {
		...
		beforeSaveRow: function (options, rowid){
			...
			let editurl = "./ptcare_doctornoteED_MR/form?"+
				$.param({
					_token: $('#_token').val(),
...
var dialog_icd = new ordialog(
	'icdcode',['hisdb.diagtab AS dt','sysdb.sysparam AS sp'],"#formDoctorNote input[name='icdcode']",errorField,
	{
		...
		urlParam: {
			url: "ptcare_doctornoteED_MR/table",
		...
		title: "Select ICD",
		open: function (){
			dialog_icd.urlParam.url = "ptcare_doctornoteED_MR/table";
...
function saveForm_doctorNote(callback){
	...
	$.post("./ptcare_doctornoteED_MR/form?"+$.param(saveParam), $.param(postobj)+'&'+$.param(values), function (data){
...
$('#tab_doctornote').on('shown.bs.collapse', function (){
	...
	docnote_date_tbl.ajax.url("./ptcare_doctornoteED_MR/table?"+$.param(dateParam_docnote)).load(function (data){
		emptyFormdata_div("#formDoctorNote",['#mrn_emergencyMain','#episno_emergencyMain']);
		// $('#docnote_date_tbl tbody tr:eq(0)').click(); // to select first row
	});
...
$('#docnote_date_tbl tbody').on('click', 'tr', function (){ 
	...
	$.get("./ptcare_doctornoteED_MR/table?"+$.param(doctornote_docnote), function (data){
...
function populate_doctornote_getdata(){
    ...
    $.get("./ptcare_doctornoteED_MR/table?"+$.param(saveParam), $.param(postobj), function (data){
