...
$(document).ready(function () {
	...
	$("#jqGridAddNotesNursingED").jqGrid({
		datatype: "local",
		editurl: "./ptcare_nursing_MR/form",
		colModel: [
	...
	var myEditOptions_addNursingED = {
		...
		beforeSaveRow: function (options, rowid){
			...
			let editurl = "./ptcare_nursing_MR/form?"+
				$.param({
					_token: $('#_token').val(),
...
function saveForm_ti(callback){
    ...
    $.post( "./ptcare_nursing_MR/form?"+$.param(saveParam), $.param(postobj)+'&'+$.param(values) , function( data ) {
...
function getdata_nursing(){
	...
	$.post( "./ptcare_nursing_MR/form?"+$.param(saveParam), $.param(postobj), function( data ) {
